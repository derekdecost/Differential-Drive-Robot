...
class DriveStateController:
    def __init__(self, input_topic, output_topic, input_type, output_type, queue_val):
        ...
        self.reset_rotation_calculator = Bool()
        self.reset_rotation_calculator.data = False
...
if __name__ == "__main__":
    try:
        ...
        while not rospy.is_shutdown():
            if drive_state_controller.drive_state.data in [1, 5, 9, 13]:
                if (drive_state_controller.r_wheel_rotations >= straight_path_params.WHEEL_ROTATIONS_TO_COMPLETE_RIGHT_PATH) and (drive_state_controller.l_wheel_rotations >= straight_path_params.WHEEL_ROTATIONS_TO_COMPLETE_LEFT_PATH):
                    drive_state_controller.drive_state.data = drive_state_controller.drive_state.data + 1
                drive_state_controller.drive_state_pub.publish(drive_state_controller.drive_state)
            
            elif drive_state_controller.drive_state.data in [3, 7, 11, 15]:
                if (drive_state_controller.r_wheel_rotations >= robot_params.LEFT_TURN_WHEEL_ROTATIONS) and (drive_state_controller.l_wheel_rotations >= robot_params.LEFT_TURN_WHEEL_ROTATIONS):
                    drive_state_controller.drive_state.data = drive_state_controller.drive_state.data + 1
                drive_state_controller.drive_state_pub.publish(drive_state_controller.drive_state)
            
            elif drive_state_controller.drive_state.data in [2, 4, 6, 8, 10, 12, 14, 16]:
                drive_state_controller.drive_state.data = drive_state_controller.drive_state.data + 1
                drive_state_controller.drive_state_pub.publish(drive_state_controller.drive_state)
